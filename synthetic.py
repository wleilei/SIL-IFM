...
############## Data Loading ##############
from data import SubGraphData,SubClassData, get_indices, get_dataset
from torch_geometric.loader import DataLoader

def load_data(data_dir, num_parts, train_ratio, test_ratio, min_length, class_batch_size, num_workers):    
    dataset, num_classes = get_dataset(data_dir)
    train_indices, val_indices, test_indices = get_indices(num_parts, train_ratio, test_ratio)
    # train_dataloader
    train_data = SubGraphData(dataset, num_parts, train_indices,True)
    train_dataloader = DataLoader(train_data, batch_size=1, shuffle=True, num_workers=8, pin_memory=True)
    nodes_by_label = train_data.nodes_by_label
    val_data = SubGraphData(dataset, num_parts, val_indices)
    val_dataloader = DataLoader(val_data, batch_size=1, shuffle=False, num_workers=8, pin_memory=True)
    # test_dataloader
    test_data = SubGraphData(dataset, num_parts, test_indices)
    test_dataloader = DataLoader(test_data, batch_size=1, shuffle=False, num_workers=8, pin_memory=False)
    # class_dataloader
...
def train(wb,epochs,device,train_dataloader,val_dataloader,class_dataloader,model,optimizer,scheduler,num_classes,invar,model_dir):
    ...
    for epoch in range(1,epochs+1):
        ...
        if wb:
            wandb.log({"val_macro_f1_mean":val_f1_mean,
        ...
        if patience_counter >= patience:
            ...
            break
    logging.info(f'name: {arg.model_config_reddit["convs"]}-400-{invar}, '
...
def test(device,test_dataloader,model,num_classes,invar):
    ...
    for step,graph in enumerate(tqdm(test_dataloader,desc=f"testing",unit='batch')):
        ...
        if invar:
            ...
            x_h = model(node=x_n,edge_index=edge_index)
            logits = torch.softmax(x_h,dim=1)
        p = torch.argmax(logits, dim=1)
        macro_f1_score = f1_weighted(p, y_n)
...
def main():
    ...
    num_classes = arg.model_config_reddit["num_cls"]
    train_dataloader, val_dataloader, test_dataloader, class_dataloader, H = load_data(**arg.data_config_reddit)
    model = load_model(**arg.model_config_reddit,H=H)
    invar = arg.model_config_reddit["invar"]
    class_batch_size = arg.data_config_reddit["class_batch_size"]
    model.to(device)
    ...
    model_dir = f"models/{ arg.model_config_reddit['convs']}-{class_batch_size}.pth"
